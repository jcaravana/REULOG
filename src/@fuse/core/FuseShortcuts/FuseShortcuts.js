...
function FuseShortcuts(props) {
  ...
  const shortcut = useSelector(({ user }) => user.shortcuts);
  const shortcutItems = shortcut
    ? shortcut.map((id) => navigation.find((item) => item.id === id))
    : [];
      ...
      {useMemo(() => {
        ...
        return (
          ...
          >
            ...
            <Tooltip
              title="Clique para adicionar/remover um atalho"
              placement={props.variant === 'horizontal' ? 'bottom' : 'left'}
            >
              <IconButton
                component={motion.div}
      ...
      >
        <div className="p-16 pt-8">
          <Input
            ...
            value={searchText}
            onChange={search}
            placeholder="Procure por um menu"
            className=""
            fullWidth
            inputProps={{
              'aria-label': 'Search',
